# No need to edit this file.  Edit skin.py.  This file is really only for addon.ini search and browse
...
from skin import ADDONID
from skin import ADDON
from skin import addonPath
from skin import basePath
from skin import ADDONID_CORE
from skin import ADDON_CORE
from skin import addonPath_core
from skin import basePath_core
from skin import extrapath
from skin import tmp_path
from skin import tmp_File
dialog    = xbmcgui.Dialog()
dp        = xbmcgui.DialogProgress()

'''
try: from skin import ADDONID
except ImportError: ADDONID = 'script.tvguide.fullscreen.skin.cake'   # set this to your skin addonid
#
try: from skin import ADDONID_CORE
...
###########################################
@plugin.route('/skin_mod_settings_minimal')
def skin_mod_settings_minimal():
    import skin;skin.skin_settings_minimal()

###########################################
#### Set tvgfs FULL skin settings
###########################################
@plugin.route('/skin_mod_settings')
def skin_mod_settings():
    import skin;skin.skin_settings()
...
@plugin.route('/open_settings_core')
def open_settings_core():  xbmcaddon.Addon(id=ADDONID_CORE).openSettings();sys.exit(0)

# Menu open this addon settings
@plugin.route('/open_settings')
def open_settings():  xbmcaddon.Addon(id=ADDONID).openSettings();sys.exit(0)

# Refresh addon subs bult in
@plugin.route('/refreshini')
def refreshini():
    xbmc.executebuiltin('RunScript(special://home/addons/'+ADDONID+'/skin.py,101)')
...
def index():
    items = []
    context_items = []
    items.append({'label': '[COLOR gold]EPG:[/COLOR]  Run  '+ADDONID_CORE+ ' EPG','path': plugin.url_for('guide'),'thumbnail':get_icon_path('icon'),})
    ...
    items.append({'label': '[COLOR yellow]Open Settings[/COLOR]  '+ADDONID_CORE,'path': plugin.url_for('open_settings_core'),'thumbnail':get_icon_path('icon'),})
    items.append({'label': '[COLOR yellow]Open Settings[/COLOR]  '+ADDONID,'path': plugin.url_for('open_settings'),'thumbnail':get_icon_path('icon'),})
    items.append({'label': '[COLOR green]SKIN:[/COLOR]  Refresh built-in Addons  ','path': plugin.url_for('refreshini'),'thumbnail':get_icon_path('icon'),})
    items.append({'label': '[COLOR green]SKIN:[/COLOR]  Set Skin for : '+ADDONID_CORE,'path': plugin.url_for('skin_mod_settings_minimal'),'thumbnail':get_icon_path('icon'),})
    items.append({'label': '[COLOR green]SKIN:[/COLOR]  Set Skin with FULL settings for : '+ADDONID_CORE,'path': plugin.url_for('skin_mod_settings'),'thumbnail':get_icon_path('icon'),})
    ADDON.setSetting('firstrun','false')
    xmltoset = ADDON.getSetting('xmltv.url')
    if not ADDON.getSetting('xmltv.url') == '': items.append({'label': '[COLOR green]EPG:[/COLOR] '+xmltoset,'path': plugin.url_for('skin_mod_settings'),'thumbnail':get_icon_path('icon'),})
    return items
